...
Look at the **samples** included in our dataset using [bcftools](https://samtools.github.io/bcftools/bcftools.html) a toolset for variant calling and manipulating VCF (Variant Call Format) and BCF (Binary Call Format) files, including tasks like viewing, merging, and querying the data. The `query` option of bcftools is very useful for exploring the vcf/bcf. Look at the [`query` manual page](https://samtools.github.io/bcftools/bcftools.html#query). 
